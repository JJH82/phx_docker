...
RUN microdnf install -y git make gcc gcc-c++ glibc-langpack-en ncurses openssl unzip && microdnf clean all
...
COPY --from=builder /root/.cache/rebar3 /root/.cache/rebar3FROM registry.access.redhat.com/ubi8/ubi-minimal AS builder
